  ...
  if (!dbConfig.configured) {
    return new Response(
      JSON.stringify({
        ...
        message: "请配置 Upstash Redis (env.UPSTASH_REDIS_REST_URL + env.UPSTASH_REDIS_REST_TOKEN)、KV 存储 (env.img_url) 或 D1 数据库 (env.img_d1)。 / Please configure Upstash Redis (env.UPSTASH_REDIS_REST_URL + env.UPSTASH_REDIS_REST_TOKEN), KV storage (env.img_url) or D1 database (env.img_d1)."
      }),
      {
        status: 500,
        headers: {
